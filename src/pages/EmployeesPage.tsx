    ...
    const fetchEmployees = useCallback(async (p: number, s: string, d: string, inactive = false) => {
        ...
        try {
            const res = await employeeService.list({
                ...
                // Supabase takes a tri-state filter where the old API took a
                // boolean. "all" is the nearest equivalent of include_inactive:
                // it returns leavers alongside current staff rather than only
                // the leavers.
                status: inactive ? "all" : "active",
            });
